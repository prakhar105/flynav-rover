...
# ============================================================
# STEP 44 - COMBINED NAVIGATION V1 RANDOMIZED ROBUSTNESS TEST
#
# Reproducible randomized Webots validation for the frozen
# controllers/flynav_combined_v1/flynav_combined_v1.py.
#
# This script DOES NOT retune or modify the frozen controller.
# It imports and executes the same combined control functions.
#
# Default:
#   30 randomized trials
#   deterministic seed
#   mixture of clear/direct/offset/near-wall obstacle layouts
# ============================================================
...
# Arena in the current world is approximately 1 m x 1 m,
# centered at (0, 0). Sampling stays comfortably inside it.
SAMPLE_LIMIT = 0.36
...
# Trial mix. The list is sampled randomly but reproducibly.
SCENARIO_WEIGHTS = [
    ("clear", 0.15),
    ("direct", 0.35),
    ("offset", 0.35),
...
def load_combined_module(project_root):
    ...
    if not source.exists():
        raise FileNotFoundError(
            "Could not find frozen combined controller:\n"
            f"{source}"
        )
...
def build_random_trial(rng, trial_index):
    ...
    if scenario == "near_wall":
        # Horizontal or vertical corridor close to one wall,
        # but with enough physical room for the E-puck.
    ...
    # Perpendicular to start->goal line.
    px = -uy
    py = ux

    if scenario == "clear":
        # Move obstacle well away from the intended corridor.
        ...
        # If that lands too close to edge, use a safe corner-ish
        # location away from both start and goal.
        if not point_inside_sampling_area(ox, oy, margin=0.02):
            candidates = [
                (-0.30, -0.30),
                (-0.30, +0.30),
                (+0.30, -0.30),
        ...
        # Obstacle placed along the route.
        ...
        # Keep obstacle inside safe placement region.
        ox = clamp(
            ox,
            -SAMPLE_LIMIT + 0.04,
            +SAMPLE_LIMIT - 0.04,
    ...
    # Start heading is fully randomized. This deliberately
    # combines the previously validated heading behavior with
    # obstacle arbitration.
...
def main():
    ...
    if self_node is None:
        raise RuntimeError(
            "Step 44 requires supervisor TRUE."
        )
    ...
    obstacle_node = find_test_obstacle(
        robot
    )
    ...
    if goal_node is not None:
        goal_translation = goal_node.getField(
            "translation"
        )
    ...
    output_file = (
        ...
        / "connectome"
        / "output"
        / "combined_navigation_v1_randomized.csv"
    )
    ...
    print("=" * 118)
    print(
        "STEP 44 - COMBINED NAVIGATION V1 "
    ...
    try:
        for trial in trials:
            ...
            while elapsed_s < TRIAL_TIMEOUT_S:
                ...
                if obstacle_latched:
                    blend = (
                        combined.obstacle_blend_weight(
                            max_sensor_raw
                        )
                    )
        ...
        if failures:
            ...
            print()
            print(
                "Do not retune immediately. Re-run the same "
                "seed first, then inspect the repeated failures."
            )

        else:
            print()
            print(
                "PASS: all randomized trials reached their goals."
            )
            print(
                "NEXT: repeat with additional seeds / a larger "
                "trial count before claiming broad robustness."
            )
